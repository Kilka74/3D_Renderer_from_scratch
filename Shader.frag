...
uniform vec3 u_pla_norm;
uniform vec4 u_pla_col;
...
float[9] ObjectIntersection(vec3 ro, vec3 rd) {
    vec4 col;
...
vec4 castRay(inout vec3 ro, inout vec3 rd) {
    ...
    float data[9] = ObjectIntersection(ro, rd);
    ...
    vec3 planeNorm = normalize(u_pla_norm);
